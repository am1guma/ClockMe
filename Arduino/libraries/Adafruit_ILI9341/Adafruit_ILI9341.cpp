...
void Adafruit_ILI9341::bmpDraw(char *filename, int16_t x, int16_t y) {
    ...
    if (read16(bmpFile) == 1) { // # planes -- must be '1'
      ...
      if ((bmpDepth == 24) && (read32(bmpFile) == 0)) { // 0 = uncompressed
        ...
        if ((x2 >= 0) && (y2 >= 0)) { // On screen?
          ...
          startWrite(); // Requires start/end transaction now
          setAddrWindow(x, y, w, h);
...
uint16_t Adafruit_ILI9341::read16(File &f) {
  uint16_t result;
...
uint32_t Adafruit_ILI9341::read32(File &f) {
  uint32_t result;
  ((uint8_t *)&result)[0] = f.read(); // LSB
